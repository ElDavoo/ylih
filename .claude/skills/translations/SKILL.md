...
| `lingua franca: N/M of X’s commonest grammatical words` | a locale holding the language of the nearest city instead of its own. Four Chagga locales held one Swahili translation between them; `values-b+vmw` held Chichewa under a Makhuwa name. Invisible to every rule above — the strings are present, the script is right, the text is not English, and the language it really holds is usually not in the tree to compare against. Grammatical words only, since the Arabic-derived vocabulary Swahili is famous for is exactly what neighbours borrow legitimately. `except` per fingerprint is the set allowed to match: the language itself, plus a relative close enough to share that much grammar (`seh` is the one so far) |
